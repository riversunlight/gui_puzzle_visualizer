...
void next_go(int& next_x, int& next_y, int &sel_x, int &sel_y, char &s, picture &pic) {
	switch (s) {
	case 'R':
...
vector<int> change_rotate(answer &ans, picture &pic) {
	...
	vector<int> replace(pic.div_y * pic.div_x);
	...
	for (int i = 0; i < ans.sel_place.size(); i++) {
		...
		for (int j = 0; j < ans.move_operate[i].size(); j++) {
			next_go(next_x, next_y, sel_x, sel_y, ans.move_operate[i][j], pic);
	...
	for (int i = 0; i < pic.div_x * pic.div_y; i++)
		replace[final_place[i]] = i;
	for (int i = 0; i < pic.div_x * pic.div_y; i++)
		res[i] = ans.rotate[replace[i]];
	return res;
}
